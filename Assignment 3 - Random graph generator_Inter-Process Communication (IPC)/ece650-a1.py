...
def print_vertices_edges(vertices_final, edges_f):
    ...
    print("V "+ str(len(vertices_final)))
...
def main():
    while True:
        ...
        try:
            ...
            # if cmd_a_chk.match(cmd) and obj_street.street_search() != 1:
            ...
            #     vertices_compute()
            #     #sys.stderr.write("Error: Please check for missing/additional quotes, brackets, spaces, commands etc.., and try again") 
            ...
            #     print("Error: Street name is not in the right format, please input again",file=sys.stderr)
            ...
            #     print("Error: Input should have at least two coordinates, please try again",file=sys.stderr)
            # elif not cmd_g_chk.match(cmd) and name_upd[1][0] == " " :
            #     print("Error: Check for leading spaces and please try again",file=sys.stderr)
            # elif not cmd_g_chk.match(cmd) and name_space_chk.findall(name_upd[1]):
            #     print("Error: Check spaces in between street name and please try again",file=sys.stderr)
            # elif cmd_a_chk.match(cmd) and obj_street.street_search() != 1:
            #     #streets.append(name_upd[1])
            #     Street.street_storing(name_upd[1],coords)
            #     #obj_street.lines_compute()
            # elif cmd_a_chk.match(cmd) and obj_street.street_search() == 1:
            #     print("Error: Street already exists, Please try again",file=sys.stderr)
            # elif (cmd_m_chk.match(cmd) or cmd_r_chk.match(cmd)) and obj_street.street_search() != 1:
            #     print("Error: Street doesn't exist, please try again",file=sys.stderr)
            # elif cmd_m_chk.match(cmd) and obj_street.street_search() == 1:
            #     modify_street(name_upd[1], coords)
            # elif cmd_r_chk.match(cmd) and obj_street.street_search() == 1:
            #     delete_street(name_upd[1])
            ...
            #     vertices_compute()
            # else:
            #     print("Error: Please check for missing/additional quotes, brackets, spaces, commands etc.., and try again",file=sys.stderr)
            #     #sys.stderr.write("Error: Please check for missing/additional quotes, brackets, spaces, commands etc.., and try again")     

            if cmd[0:3] == 'add' and obj_street.street_search() != 1:
                #streets.append(name_upd[1])
                Street.street_storing(name_upd[1],coords)
                #obj_street.lines_compute()
            elif cmd[0:3] == 'mod' and obj_street.street_search() == 1:
                modify_street(name_upd[1], coords)
            elif cmd[0:2] == 'rm' and obj_street.street_search() == 1:
                delete_street(name_upd[1])
            elif cmd[0:2] == 'gg':
                # print("in gg")
                # obj_street.lines_compute()
                ...
                print("Error: Invalid input, please try again",file=sys.stderr)    
